...
class UserSearchTableViewController<Cell: UITableViewCell where Cell: CellProtocol>: TableViewController<Cell>, UISearchBarDelegate {
  ...
  override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
    if let user = self.data[indexPath.row] as? User {
      let next = RepositoriesTableViewController<RepositoryTableViewCell>()
